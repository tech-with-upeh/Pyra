...
var ASM_CONSTS = {
  85564: ($0) => { document.body.style = allocateUTF8(""); document.body.innerHTML = UTF8ToString($0); },  
 85652: ($0) => { document.title = UTF8ToString($0); },  
 85691: ($0) => { if (!document.getElementById("__ink_styles")) { const style = document.createElement("style"); style.id = "__ink_styles"; style.innerHTML = UTF8ToString($0); document.head.appendChild(style); } },  
 85889: ($0, $1) => { document.body.setAttribute(UTF8ToString($0), UTF8ToString($1)); },  
 85957: ($0, $1) => { const id = UTF8ToString($0); if (!document.getElementById(id)) { document.body.insertAdjacentHTML("beforeend", UTF8ToString($1)); } },  
 86093: () => { function rafLoop() { Module._animatefps(); requestAnimationFrame(rafLoop); } requestAnimationFrame(rafLoop); },  
 86206: ($0) => { window.addEventListener(UTF8ToString($0), function () { Module._handleEvent($0); }); },  
 86295: () => { Module._handleRoute(allocateUTF8(window.location.pathname)); window.addEventListener("popstate", () => { Module._handleRoute(allocateUTF8(window.location.pathname)); }); },  
 86469: () => { return window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight; },  
 86571: () => { return window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth; },  
 86670: ($0, $1) => { const ctx = document.getElementById(UTF8ToString($0))?.getContext("2d"); if (!ctx) return; ctx.strokeStyle = UTF8ToString($1); },  
 86801: ($0, $1) => { const ctx = document.getElementById(UTF8ToString($0))?.getContext("2d"); if (!ctx) return; ctx.lineWidth = $1; },  
 86916: ($0, $1, $2, $3, $4) => { const ctx = document.getElementById(UTF8ToString($0))?.getContext("2d"); if (!ctx) return; ctx.beginPath(); ctx.moveTo($1, $2); ctx.lineTo($3, $4); ctx.stroke(); }
};

// Imports from the Wasm binary.
var _invokeVNodeCallback = Module['_invokeVNodeCallback'] = makeInvalidEarlyAccess('_invokeVNodeCallback');
var _js_insertHTML = Module['_js_insertHTML'] = makeInvalidEarlyAccess('_js_insertHTML');
var _handleRoute = Module['_handleRoute'] = makeInvalidEarlyAccess('_handleRoute');
var _js_setTitle = Module['_js_setTitle'] = makeInvalidEarlyAccess('_js_setTitle');
var _js_insertCSS = Module['_js_insertCSS'] = makeInvalidEarlyAccess('_js_insertCSS');
var _js_setBodyAttr = Module['_js_setBodyAttr'] = makeInvalidEarlyAccess('_js_setBodyAttr');
var _allocateString = Module['_allocateString'] = makeInvalidEarlyAccess('_allocateString');
...
var _animatefps = Module['_animatefps'] = makeInvalidEarlyAccess('_animatefps');
var _js_reqfps = Module['_js_reqfps'] = makeInvalidEarlyAccess('_js_reqfps');
var _handleEvent = Module['_handleEvent'] = makeInvalidEarlyAccess('_handleEvent');
var _js_addpageEventlisteners = Module['_js_addpageEventlisteners'] = makeInvalidEarlyAccess('_js_addpageEventlisteners');
