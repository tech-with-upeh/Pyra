...
var ASM_CONSTS = {
  85580: ($0) => { document.body.style = allocateUTF8(""); document.body.innerHTML = UTF8ToString($0); },  
 85668: ($0) => { document.title = UTF8ToString($0); },  
 85707: ($0) => { if (!document.getElementById("__ink_styles")) { const style = document.createElement("style"); style.id = "__ink_styles"; style.innerHTML = UTF8ToString($0); document.head.appendChild(style); } },  
 85905: ($0, $1) => { document.body.setAttribute(UTF8ToString($0), UTF8ToString($1)); },  
 85973: ($0, $1) => { const id = UTF8ToString($0); if (!document.getElementById(id)) { document.body.insertAdjacentHTML("beforeend", UTF8ToString($1)); } },  
 86109: () => { function rafLoop() { Module._animatefps(); requestAnimationFrame(rafLoop); } requestAnimationFrame(rafLoop); },  
 86222: ($0) => { window.addEventListener(UTF8ToString($0), function () { Module._handleEvent($0); }); },  
 86311: () => { Module._handleRoute(allocateUTF8(window.location.pathname)); window.addEventListener("popstate", () => { Module._handleRoute(allocateUTF8(window.location.pathname)); }); },  
 86485: () => { return window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight; },  
 86587: () => { return window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth; },  
 86686: ($0, $1) => { const ctx = document.getElementById(UTF8ToString($0))?.getContext("2d"); if (!ctx) return; ctx.strokeStyle = UTF8ToString($1); },  
 86817: ($0, $1) => { const ctx = document.getElementById(UTF8ToString($0))?.getContext("2d"); if (!ctx) return; ctx.lineWidth = $1; },  
 86932: ($0, $1, $2, $3, $4) => { const ctx = document.getElementById(UTF8ToString($0))?.getContext("2d"); if (!ctx) return; ctx.beginPath(); ctx.moveTo($1, $2); ctx.lineTo($3, $4); ctx.stroke(); }
};
